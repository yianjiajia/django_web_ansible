...
from devops.apps.ansible.tasks import get_allusers
from devops.apps.ansible.tasks import sendmail, getfilecontent, savefilecontent
from devops.apps.ansible.tasks import get_hosts, get_sshkey_deploy
from devops.lib.django_util import render, render_json
...
def execute_script(request, project_name):
    ...
    if request.method == 'POST':
        ...
        return HttpResponseRedirect(
            urlresolvers.reverse('job_execute_result', kwargs={'project_name': project_name, 'job_pk': job_pk}))
...
def result_json(request, project_name, job_pk):
    ...
    data = {'status': job.status, 'result_stdout': job.result_stdout, 'result_stderr': job.result_stderr,
