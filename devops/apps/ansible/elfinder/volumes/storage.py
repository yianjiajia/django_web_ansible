...
from devops.apps.ansible.elfinder.exceptions import NotAnImageError, ElfinderErrorMessages
...
class ElfinderVolumeStorage(ElfinderVolumeDriver):
        ...
        if not 'storage' in opts:
            if not 'storageClass' in opts:
                ...
                try:
                    ...
                    raise Exception(
                        'Storage %s should implement both the listdir() and url() methods to be valid for use with yawd-elfinder.' %
                        self._options['storage'].__class__)
    ...
    def _configure(self):
        ...
        if not self._isabs(self._options['tmbPath']):
            ...
            if not self._options['tmbURL'] and self._options['URL']:
                self._options['tmbURL'] = self._options['URL'] + self._options['tmbPath'][len(self._root) + 1:].replace(
                    self._separator, '/') + '/'
    ...
    def _extract(self, path, archiver):
        ...
        if self._options['archiveMaxSize'] > 0 and self._options['archiveMaxSize'] < self._local_dir_size(
                quarantine_dir):
        ...
        if len(ls) >= 1:
            ...
            name = archive_name
            m = re.search(r'\.((tar\.(gz|bz|bz2|z|lzo))|cpio\.gz|ps\.gz|xcf\.(gz|bz2)|[a-z0-9]{1,4})$', name,
                          re.IGNORECASE)
            if m and m.group(0):
