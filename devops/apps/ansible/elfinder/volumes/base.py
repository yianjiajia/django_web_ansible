...
from devops.apps.ansible.elfinder.exceptions import ElfinderErrorMessages, FileNotFoundError, DirNotFoundError, \
    PermissionDeniedError, NamedError, NotAnImageError
from devops.apps.ansible.elfinder.utils.archivers import ZipFileArchiver
...
class ElfinderVolumeDriver(object):
    ...
    def mime_accepted(self, mime, mimes=[], empty=True):
        ...
        return mime == 'directory' or 'all' in mimes or 'All' in mimes or mime in mimes or mime[
                                                                                           0:mime.find('/')] in mimes
    ...
    def tmb(self, hash_):
        ...
        if s[0] <= tmb_size and s[1] <= tmb_size:
            ...
            if s[0] > tmb_size and s[1] > tmb_size:
                ...
                s = resized.size
                self._img_crop(resized, tmb, tmb_size, tmb_size, int((s[0] - tmb_size) / 2), int((s[1] - tmb_size) / 2),
                               'png')
            else:
                fit = self._img_square_fit(im, None, s[0] if s[0] > s[1] else s[1], s[0] if s[0] > s[1] else s[1],
    ...
    def paste(self, volume, hash_src, dst, rm_src=False):
        ...
        if self.command_disabled('paste') or (volume == self and rm_src and self.command_disabled('rm')) or (
        ...
        if file_['mime'] == 'directory' and ((volume == self and rm_src and self.command_disabled('rmdir')) or (
                    volume != self and rm_src and volume.command_disabled('rmdir'))):
            raise PermissionDeniedError
        ...
        try:
            ...
            if self._options['copyOverwrite']:
                ...
                if (file_['mime'] == 'directory' and stat['mime'] != 'directory') or (
                        file_['mime'] != 'directory' and stat['mime'] == 'directory'):
                ...
                if volume == self and (
                    ('target' in file_ and test == file_['target']) or test == self.decode(hash_src)):
    ...
    def stat(self, path):
        ...
        if stat_cache is None or root_cache != self._root:
                ...
                if stat['mime'] == 'directory':  # handle directories
                    if self._options['checkSubfolders']:
                        if 'dirs' in stat:
                            ...
                            stat['dirs'] = int('dirs' in stat_cache[stat['target']]) if stat[
                                                                                            'target'] in stat_cache else int(
            ...
            if self._options['cache']:
                ...
                self.logger.debug('%s: test Caching STAT %s' % (self.id(), path))
    ...
    def _size(self, path):
        ...
        for stat in self._get_scandir(path):
            size = self._size(self._join_path(path, stat['name'])) if stat['mime'] == 'directory' and stat['read'] else \
            stat['size']
            if (size > 0):
                result += size
    ...
    def _can_create_tmb(self, path, stat):
        ...
        """
        return self._tmb_path_writable and not path.startswith(self._options['tmbPath']) and stat['mime'].startswith(
            'image')
    ...
    def _checkArchivers(self):
        ...
        if 'create' in self._options['archivers']:
            for mime, archiver in self._options['archivers']['create'].items():
                ...
                if re.match(r'application/', mime) and inspect.isclass(conf) and hasattr(conf, 'open') and callable(
                        getattr(conf, 'open')) and hasattr(conf, 'add') and callable(getattr(conf, 'add')) and hasattr(
                        conf, 'close') and callable(getattr(conf, 'close')):
        ...
        if 'extract' in self._options['archivers']:
            for mime, archiver in self._options['archivers']['extract'].items():
                ...
                if re.match(r'application/', mime) and inspect.isclass(conf) and hasattr(conf, 'open') and callable(
                        getattr(conf, 'open')) and hasattr(conf, 'extractall') and callable(
                        getattr(conf, 'extractall')) and hasattr(conf, 'close') and callable(getattr(conf, 'close')):
